...
**TODO:** Сделать в качестве опции передачу параметра logger, при установке которого будут выводится отладочные сообщения. Если в качестве значения параметра будет передана функция - вывод будет осуществляться через нее, если любое другое значение, приводимое к `true`, то для вывода `console.log`. Значение по умолчанию - false
